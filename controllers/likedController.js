...
module.exports.getLikedPet = function(req, res){
    ...
    self.alternatives = function () {
        var getLiked = "SELECT pet.id, pet.pet_name, DATE_FORMAT(pet.pet_dob, '%Y-%m-%d') AS pet_dob, pet.pet_sex, pet.furcolor, pet.weight, pet.breed, breeds.name AS breed_name, breeds.size, variants.id AS variant_id, variants.name AS variant, pet.pet_photo, pet.breed_cert, pet.pet_desc, pet.user_id, user_profile.name, user.username,  DATE_FORMAT(user_profile.user_dob, '%Y-%m-%d') AS user_dob, user_profile.photo, user_profile.sex, regencies.name AS city, provinces.name AS provinces, have_vaccines.id_vaccine, vaccines.name AS name_vaccine, breeds.mixed FROM `pet` JOIN `user_profile`ON pet.user_id = user_profile.id JOIN user ON user.id = user_profile.username_id JOIN breeds ON breeds.id = pet.breed JOIN regencies ON regencies.id = user_profile.city JOIN provinces ON regencies.province_id = provinces.id JOIN variants ON variants.id = breeds.variant JOIN liked ON pet.id = liked.to LEFT JOIN have_vaccines ON have_vaccines.id_pet = pet.id LEFT JOIN vaccines ON vaccines.id = have_vaccines.id_vaccine LEFT JOIN (SELECT history_with.pet_to FROM history_with WHERE history_with.pet_from = "+res.locals.pet_id+") history ON pet.id = history.pet_to WHERE liked.from = "+res.locals.pet_id+" AND liked.like_stat = 1 AND history.pet_to IS NULL";
    ...
    self.srcPet = function (results) {
        var srcSql = "SELECT pet.id, pet.pet_name, DATE_FORMAT(pet.pet_dob, '%Y-%m-%d') AS pet_dob, pet.pet_sex, pet.furcolor, pet.weight, pet.breed, breeds.name AS breed_name, breeds.size, variants.name AS variant, pet.pet_photo, pet.breed_cert, pet.pet_desc, pet.user_id, user_profile.name, user.username, DATE_FORMAT(user_profile.user_dob, '%Y-%m-%d') AS user_dob, user_profile.photo, user_profile.sex, regencies.name AS city, provinces.name AS provinces, have_vaccines.id_vaccine, vaccines.name AS name_vaccine, breeds.cross_possibility, breeds.mixed FROM `pet` JOIN `user_profile`ON pet.user_id = user_profile.id JOIN user ON user.id = user_profile.username_id JOIN breeds ON breeds.id = pet.breed JOIN regencies ON regencies.id = user_profile.city JOIN provinces ON regencies.province_id = provinces.id JOIN variants ON variants.id = breeds.variant LEFT JOIN have_vaccines ON have_vaccines.id_pet = pet.id LEFT JOIN vaccines ON vaccines.id = have_vaccines.id_vaccine WHERE pet.id = "+res.locals.pet_id+"";
