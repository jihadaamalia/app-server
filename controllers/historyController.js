...
module.exports.getSelfHistory = function (req,res) {
    var self = this;
    var ownPet = "SELECT history_with.pet_from, pet_f.pet_name AS from_name, pet_f.pet_photo AS from_photo, breeds_f.name AS from_breed, history_with.pet_to, pet_t.pet_name AS to_name, pet_t.pet_photo AS to_photo, breeds_t.name AS to_breed, history_with.match_stat, history_with.match_date, history_with.score FROM history_with JOIN pet pet_f ON history_with.pet_from = pet_f.id JOIN pet pet_t ON history_with.pet_to = pet_t.id JOIN breeds breeds_f ON breeds_f.id = pet_f.breed JOIN breeds breeds_t ON breeds_t.id = pet_t.breed WHERE history_with.pet_from = '"+res.locals.pet_id+"'"
